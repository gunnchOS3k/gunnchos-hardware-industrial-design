...
hardware-v1-generate:
	$(PYTHON) scripts/generate_hardware_v1c_campaign.py
...
hardware-v1-gates:
	...
	@$(PYTHON) -c "import json; g=json.load(open('hardware_v1/GATES.json')); keys=['CUSTOM_MAINLINE_ARCHITECTURE_FROZEN','CUSTOM_PLATFORM_VENDOR_ACCESS_READY','CUSTOM_SOC_PINMAP_UNDERSTOOD','CUSTOM_POWER_TREE_UNDERSTOOD','CUSTOM_DDR_TOPOLOGY_UNDERSTOOD','CUSTOM_PCIE_TOPOLOGY_UNDERSTOOD','CUSTOM_USB4_TOPOLOGY_UNDERSTOOD','CUSTOM_DISPLAY_TOPOLOGY_UNDERSTOOD','CUSTOM_EC_ARCHITECTURE_UNDERSTOOD','CUSTOM_BOOT_CHAIN_UNDERSTOOD','CUSTOM_DEBUG_ARCHITECTURE_UNDERSTOOD','CUSTOM_MANUFACTURING_FLOW_UNDERSTOOD','CPB0_SCHEMATIC_READY','CPB0_PCB_READY','CPB0_READY_FOR_FAB','CUSTOM_MAINLINE_READY_FOR_EVT_BUILD','HARDWARE_V1_READY_FOR_EVT_BUILD','HARDWARE_V1_DIGITAL_ARCHITECTURE_COMPLETE','REFERENCE_PLATFORM_0_READY_FOR_FAB','RP0_A_COTS_PROCUREMENT_PACKET_READY','RP0_A_READY_TO_ORDER','RP0_A_PHYSICAL_BUILD_PENDING','RP0_A_BRINGUP_PENDING','RP0_B_CUSTOM_READY_FOR_FAB','PRODUCT_CELLULAR_ANTENNA_DESIGN_PENDING','UNRES_NRF54L15_FOOTPRINT_CLOSED','UNRES_FN990B40_AVL_CLOSED','EXT_DSXL_DUAL_EDP_CLOSED','EXT_COM_HPC_400PIN_RP0_A_BLOCKING','EXT_COM_HPC_400PIN_RP0_B_BLOCKING','EXT_JHL8440_BALLMAP_RP0_A_BLOCKING','EXT_JHL8440_BALLMAP_RP0_B_BLOCKING','EXT_JHL9040R_BALLMAP_RP0_A_BLOCKING','EXT_JHL9040R_BALLMAP_RP0_B_BLOCKING','EVT_PENDING','DVT_PENDING','PVT_PENDING','PHYSICAL_HARDWARE_VALIDATED','CERTIFICATION_COMPLETE','MANUFACTURING_VALIDATED','RFQ_SENT','NEXT_OWNER_ACTION','OPTIONAL_OWNER_ACTION','NEXT_GATE','COM_HPC_IS_PRODUCT_MAINLINE','MAINLINE_COMPUTE'];\
[print(k+'='+str(g.get(k)).lower() if isinstance(g.get(k), bool) else k+'='+str(g.get(k))) for k in keys]"

hardware-v1-report:
	@test -f hardware_v1/REPORT_SECTION_24_HW1C_A_TO_Z.md
	@echo "Report: hardware_v1/REPORT_SECTION_24_HW1C_A_TO_Z.md"
