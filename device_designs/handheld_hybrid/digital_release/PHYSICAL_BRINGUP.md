# Physical bring-up — Handheld Hybrid
...
3. Firmware image hash from `firmware/manifests/handheld_hybrid_firmware_manifest.yaml` (harness YAML is not a board image).
4. Modeled rails in `device_designs/handheld_hybrid/electrical/power_tree.yaml` are **expected class only**.

## Role reminder

Mobile/docked research compute. HID/gamepad is one latency workload, not the product identity. Do not treat as generic gaming.
